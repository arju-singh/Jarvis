...
def setup_workspace(mode: str, dynamic_urls: list[str] = None) -> str:
    ...
    try:
        ...
        if mode == 'developer':
            ...
            # Open the new folder in an editor (override with EDITOR_CMD).
            antigravity_path = os.path.expanduser("~/.antigravity/antigravity/bin/antigravity")
            editor_cmd = os.environ.get("EDITOR_CMD")
            if editor_cmd:
                subprocess.run([editor_cmd, new_folder], check=False)
            elif os.path.exists(antigravity_path):
                subprocess.run([antigravity_path, new_folder], check=False)
            else:
                subprocess.run(["open", "-a", "Visual Studio Code", new_folder], check=False)
